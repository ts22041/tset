...
def page_2():
    ...
    figure4()
    st.write('-' * 50)
    st.write('서울특별시 내 각 구별 평균 연령과 후보자별 득표율을 그래프 및 표로 표현하였고 이를 바탕으로 연령에 따른 주요 후보자들의 득표율 산점도 및 추세선을 표현하였다. 이를 분석한 결과 예상과 반대로 진보 후보인 박영선 후보의 득표율이 평균 연령에 비례하는 경향을 보였고 보수 후보인 오세훈 후보의 득표율이 평균 연령에 반비례하는 경향을 보였다.')
